...
def main():
    ...
    for scan_item in scan_list:
        scan_queue.put(scan_item)
    threads = []
    for i in range(num_threads):
        t = threading.Thread(target=scan_thread, args=(scan_queue, found_channels, lock, stop_event))
        t.start()
        threads.append(t)
    try:
        while not scan_queue.empty():
            time.sleep(1)
            progress = (len(scan_list) - scan_queue.qsize()) / len(scan_list)
            update_progress(progress, 'Scanning', f'(Found {found_channels[0]} channels)')
    ...
    for t in threads:
        t.join()
    print(f'\nFound {found_channels[0]} channels.')

#------------------------------------------------------------------------------

def scan_thread(q, found_channels, lock, stop_event):
    while not stop_event.is_set():
        try:
            scan_item = q.get(timeout=1)
            if iptv_test(scan_item['ip'], scan_item['port'], timeout=1):
                write_to_playlist(scan_item['ip'], scan_item['port'], lock)
                with lock:
                    found_channels[0] += 1
            q.task_done()
        except queue.Empty:
...
def write_to_playlist(ip, port, lock):
    with lock:
